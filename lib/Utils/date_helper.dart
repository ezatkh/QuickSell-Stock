...
class DateHelper {
  // Format date to a specific format (DateTime object to string)
  ...
  static String formatTransactionDateToDate(String date) {
    try {
      ...
      return formatDateOnly(parsedDate); // Return only the date part
    } catch (e) {
  ...
  // Method to format the transaction date to both date and time (yyyy-MM-dd HH:mm)
  static String formatTransactionDate(String date) {
    try {
      DateTime parsedDate = DateTime.parse(date); // Parse the string to DateTime
      return formatDateWithoutTimeZone(parsedDate); // Return date and time
    } catch (e) {
      return date; // If parsing fails, return the original string
    }
  }

  // Method to reset time to midnight (00:00:00) for a given DateTime
  static DateTime resetTimeToMidnight(DateTime date) {
    return DateTime(date.year, date.month, date.day);  // Resets to midnight (00:00:00)
  }


  static String resetTimeFromString(String dateString, {String format = 'yyyy-MM-dd'}) {
    try {
      DateTime parsedDate = DateTime.parse(dateString);  // Parse string to DateTime
      DateTime resetDate = resetTimeToMidnight(parsedDate); // Reset time to midnight
      return formatDateOnly(resetDate);  // Return formatted date without time
    } catch (e) {
